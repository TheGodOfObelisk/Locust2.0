...
能力问题（Competency Question，CQ）是本体工程中的一个关键概念，用于验证和评估本体是否能够满足特定领域的需求。CQ 通常是一组问题，通过这些问题可以检验本体是否能够正确地表示和推理领域知识。

多源异构输入的本体学习是指从多种来源和不同结构的数据中提取知识，并构建或扩展本体的过程。这些数据源可能包括关系型数据库、非关系型数据库、API 接口、文件系统等。由于数据来源和格式的多样性，处理多源异构数据需要经过数据接入、转换、清洗和整合等步骤 。
